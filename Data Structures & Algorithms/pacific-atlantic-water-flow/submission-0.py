class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def bfs(row, col, ocean, prevH):
            ...
            bfs(row + 1, col, ocean, heights[row][col])
            bfs(row - 1, col, ocean, heights[row][col])
            bfs(row, col + 1, ocean, heights[row][col])
            bfs(row, col - 1, ocean, heights[row][col])

        ROWS, COLS = len(heights), len(heights[0])
        # Perform bfs from pacific rows to atlantic and from atlantic to pacific
        ...
        # Perform bfs top and bottom
        for col in range(COLS):
            # Top row
            bfs(0, col, pac, heights[0][col])
            pac.add((0, col))
            # Bottom row
            bfs(ROWS - 1, col, atl, heights[ROWS - 1][col])
            atl.add((ROWS -1, col))
        # Perform bfs for left and right cols
        for row in range(ROWS):
            # Left col
            bfs(row, 0, pac, heights[row][0])
            pac.add((row, 0))
            # Right col
            bfs(row, COLS - 1, atl, heights[row][COLS - 1])
            atl.add((row, COLS - 1))
